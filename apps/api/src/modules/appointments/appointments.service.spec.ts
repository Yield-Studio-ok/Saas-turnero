...
describe("Appointments Cancellation (Ticket 21)", () => {
  ...
  describe("AppointmentsService.cancelAppointment", () => {
    ...
    it("should store cancellationReason if provided", async () => {
      const result = await service.cancelAppointment("appointment-123", "Cliente solicit� reprogramaci�n");

      expect(mockDoc.update).toHaveBeenCalledWith(
        expect.objectContaining({
          status: "Cancelado",
          cancellationReason: "Cliente solicit� reprogramaci�n",
        }),
      );
      expect(result.cancellationReason).toBe("Cliente solicit� reprogramaci�n");
    });
