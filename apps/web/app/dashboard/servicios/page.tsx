...
export default function ServiciosPage() {
  ...
  const [formData, setFormData] = useState({ nombre: "", duracion: 30, precio: 0 });
  ...
  const handleOpenModal = (servicio?: any) => {
    if (servicio) {
      ...
      setFormData({ nombre: "", duracion: 30, precio: 0 });
      setEditingId(null);
    }
    setShowModal(true);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingId) {
      setServicios(
        servicios.map((serv) => (serv.id === editingId ? { ...formData, id: editingId } : serv)),
      );
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {servicios.length === 0 && (
          <div className="p-6 text-center text-gray-500">No hay servicios registrados.</div>
        )}
      </div>
